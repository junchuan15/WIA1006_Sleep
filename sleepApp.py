...
if selected == 'Introduction':
    ...
    while not valid_input:
        rem_percentage = st.slider("Percentage of REM sleep:", min_value=0.0, max_value=100.0)
        max_deep_percentage = 100.0 - rem_percentage
        deep_percentage = st.slider("Percentage of deep sleep:", min_value=0.0, max_value=max_deep_percentage, value=0.0)
        ...
        if sleep_percentage_sum != 100:
            st.warning("The sum of REM, deep, and light sleep percentages should be equal to 100. Please re-enter the values.")
        else:
            valid_input = True
    ...
    caffeine_consumption = st.number_input("Enter your caffeine consumption (in mg):", min_value=0, value=0)
    alcohol_consumption = st.number_input("Enter your alcohol consumption (in standard drinks):", min_value=0, value=0)
    smoking_status = st.selectbox("Choose your smoking status:", ("No", "Yes"))
    exercise_frequency = st.selectbox("Choose your exercise frequency (days per week):", (0,1,2,3,4,5,6,7))
    ...
    if st.button("Predict Sleep Efficiency"):
        ...
        numeric_features = ['Age', 'Sleep duration', 'REM sleep percentage', 'Deep sleep percentage', 'Light sleep percentage',
        ...
        st.markdown("<h1 style='text-align: center; font-size: 24px;'>Sleep Efficiency Percentage: {}%</h1>".format(round(predicted_sleep_efficiency[0]*100, 2)), unsafe_allow_html=True)
        st.markdown("<h2 style='text-align: center; font-size: 24px;'>{}</h2>".format(sleep_efficiency_class), unsafe_allow_html=True)
        st.write(message)
    ...
    # Occupation input
    occupation_categories = ['Accountant', 'Doctor', 'Engineer', 'Lawyer', 'Manager', 'Nurse',
    ...
    # Sleep quality input
    sleep_quality = st.slider("Set your quality of sleep (1-10):", min_value=0, max_value=10)

    # Physical activity level input
    physical_activity_level = st.slider("Set your physical activity level (1-100):", min_value=0, max_value=100)

    # Stress level input
    stress_level = st.number_input("Enter your stress level (1-10):", min_value=0, max_value=10)
    ...
    st.write("Your BMI is", weight / ((height/100) ** 2), BMI)

    # Heart rate input
    heartrate = st.number_input("Enter your heart rate (bpm):", min_value=0, max_value=100)

    # Daily steps input
    dailySteps = st.number_input("Set your daily steps:", min_value=0, max_value=30000)

    # Systolic and diastolic pressure inputs
    systolic_pressure = st.number_input("Set your systolic pressure:", min_value=0, max_value=200)
    diastolic_pressure = st.number_input("Set your diastolic pressure:", min_value=0, max_value=200)
    ...
    if st.button("Predict Sleep Disease"):
        ...
        occupation_categories = ['Occupation_Accountant', 'Occupation_Doctor', 'Occupation_Engineer', 'Occupation_Lawyer',
                        'Occupation_Manager', 'Occupation_Nurse', 'Occupation_Sales Representative', 'Occupation_Salesperson', 'Occupation_Scientist',
                        'Occupation_Software Engineer', 'Occupation_Teacher']

        # Get the index of the selected occupation
        selected_occupation_index = occupation_categories.index(user_input['Occupation'])


        # Create binary features for occupation
        occupation_features = ['Occupation_' + occ for occ in occupation_categories]
        user_input.update({occ: 1 if i == selected_occupation_index else 0 for i, occ in enumerate(occupation_features)})
        ...
        # Perform Min-Max scaling on numeric features
        numeric_features = ['Gender', 'Age', 'Sleep duration', 'Quality of Sleep',
